...
if (isset($_POST['submit'])) {
    ...
    $query = "SELECT * FROM external_contacts WHERE id = " . mysqli_escape_string($db, $_POST['id']);
    ...
    $query = "DELETE FROM external_contacts WHERE id = " . mysqli_escape_string($db, $_POST['id']);
...
<body class="menu2">
    ...
    <div class="card rounded w-50">
        ...
        <form action="" method="post" >
            <p>
                Weet u zeker dat u "<?= $contact['name']?>" wilt verwijderen uit de externe contactenlijst?
            </p>
        <br>
        <input type="hidden" name="id" value="<?= $contact['id'] ?>"/>
        <input class="btn" type="submit" name="submit" value="Verwijderen"/>
        <a href="external_contacts.php" class="btn">Annuleren</a>
        </form>
        </div>
    </div>
    </div>
